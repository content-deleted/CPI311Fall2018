...
sampler DisperseSampler = sampler_state
{
    Texture = <DisperseTexture>;
    AddressU = Wrap;
    AddressV = Wrap;
...
StandardVertexOutput EffectVertex(VertexInput input)
{
...
float4 EffectPixel(StandardVertexOutput input) : COLOR0
{
    ...
    float3 diffuse = max(dot(lightDirection, input.WorldNormal), 0) * DiffuseColor; // * tex2D(DiffuseSampler, input.UV);
...
technique Effect
{
	pass Pass1
	{
		VertexShader = compile vs_4_0 EffectVertex();
		PixelShader = compile ps_4_0 EffectPixel();
	}
}